...
def _platform_custom_cpu_lines() -> list[str]:
    ...
    for arch in ARCHITECTURES.values():
        if not arch.cpu_constraint.startswith("//modules/picblobs/platforms:"):
            continue
...
def _gen_toolchains_build() -> str:
    ...
    lines.append(
        ...
        '        "@platforms//cpu:arm",\n'
        '        "//modules/picblobs/platforms:baremetal",\n'
        "    ],\n"
        ")\n\n"
    )
...
# ============================================================
# Root .bazelrc platform configs reference
# ============================================================
...
def _gen_bazelrc_block() -> str:
    ...
    for os_name, os_def in OPERATING_SYSTEMS.items():
        for arch_name in os_def.architectures:
            ...
            lines.append(
                f"build:{cfg}{' ' * pad}--platforms=//modules/picblobs/platforms:{cfg}"
            )
        lines.append("")
    return "\n".join(lines)
...
_X25519_MIPS_COPTS = (
    "copts = select({"
    '"//modules/picblobs/platforms:mipsel32": ["-mno-explicit-relocs"], '
    '"//modules/picblobs/platforms:mipsbe32": ["-mno-explicit-relocs"], '
    '"//conditions:default": []}),'
)
...
def _payload_blob_target(stem: str, copts: str, hosted: bool) -> str:
    ...
    out = textwrap.dedent(f"""\
        pic_blob(
            name = "{stem}",
            srcs = ["{stem}.c"],
            deps = ["//modules/picblobs/src/include/picblobs:headers"],
            linker_script = "//modules/picblobs/src/linker:blob.ld",
            {copts}
        )
...
def _gen_payload_build() -> str:
    ...
    lines = [
        ...
        load("//modules/picblobs/bazel:blob.bzl", "pic_blob")
        """),
    ]
    ...
    if hosted_blobs:
        lines.append(
            'load("//modules/picblobs/src/payload:extract.bzl", "extract_bin")\n'
        )
